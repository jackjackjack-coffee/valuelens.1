/**
 * Cloudflare Worker: project1
 * - Handles /fmp API requests via Alpha Vantage
 * - Serves static assets (index.html etc.) via ASSETS binding
 *
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/fmp') {
      return handleFmp(request, env);
    }
    return env.ASSETS.fetch(request);
  }
};

async function handleFmp(request, env) {
  const corsHeaders = {
  ...
  try {
    ...
    return json({
      name: ov['Name'] || ticker, sector: ov['Sector'] || '', industry: ov['Industry'] || '',
      price, eps, div, fcf, shares, debt: totalDebt / 1e6 || 0, beta,
      g1, g2, tg: 2.5, wacc: 9, fcfm: 22,
      perT: Math.min(Math.max(pe, 10), 80), perG: Math.round(g1),
      evM: Math.min(Math.max(evEbitda, 5), 50), evMg: 30,
      grG: Math.round(g1 * 0.8), grY: 4.5,
      ddmG: div > 0 ? Math.min(g1 * 0.5, 8) : 0, ddmR: 8,
      fromApi: true, fetchedAt: Date.now(),
    }, 200, corsHeaders);
