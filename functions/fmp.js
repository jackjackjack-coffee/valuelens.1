...
export async function onRequest(context) {
  ...
  try {
    ...
    // 3. Full fundamentals — sequential calls to avoid rate limiting
    if (!ticker) return json({ error: 'ticker required' }, 400, corsHeaders);
    ...
    if (!quote?.['05. price']) {
      if (quoteData?.['Note'] || quoteData?.['Information']) {
        return json({ error: 'API rate limit reached. Please wait 1 minute and try again.' }, 429, corsHeaders);
      }
    ...
    // Net debt in $M (FIX: was incorrectly using capitalExpenditures)
