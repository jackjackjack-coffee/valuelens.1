/**
 * Cloudflare Worker: handles all stock data API requests via Alpha Vantage.
 *
 * Environment variable required (set in CF Workers dashboard under Settings > Variables):
...
export default {
  async fetch(request, env) {
    ...
    if (!key) {
      return json({ error: 'AV_API_KEY not set in Worker environment variables' }, 500, corsHeaders);
    }
    ...
    try {
      ...
      if (action === 'test') {
        ...
        if (data?.['Note'] || data?.['Information']) {
          return json({ ok: false, error: 'Rate limit reached' }, 200, corsHeaders);
        }
      ...
      // 3. Full fundamentals — sequential to avoid rate limiting
      if (!ticker) return json({ error: 'ticker required' }, 400, corsHeaders);
      ...
      if (quoteData?.['Note'] || quoteData?.['Information']) {
        return json({ error: 'API rate limit reached. Please wait and try again.' }, 429, corsHeaders);
      }
      ...
      // AV returns growth as decimal (0.082 = 8.2%) — convert if needed
